...
test('full player arrow seek clamps to duration boundaries',async()=>{const {createAudioPreviewManager}=await import('../src/audio-preview-manager.js');const audio=new FakeAudio(),manager=createAudioPreviewManager({createAudio:()=>audio});manager.prepareFull({id:'a',url:'a.wav'});audio.currentTime=2;manager.seekBy({id:'a',url:'a.wav'},-5);assert.equal(audio.currentTime,0);audio.currentTime=118;manager.seekBy({id:'a',url:'a.wav'},5);assert.equal(audio.currentTime,120)});
