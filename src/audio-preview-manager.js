...
export function createAudioPreviewManager({createAudio=()=>new Audio(),setTimer=setTimeout,clearTimer=clearTimeout,hoverDelay=400,previewLimit=30}={}){
  ...
  const prepareFull=item=>{cancelPending();const player=ensureAudio(),source=sourceOf(item),same=state.id===item.id&&player.src===source;if(!same){player.pause();player.src=source;try{player.currentTime=0}catch{}}publish({id:item.id,mode:'full',playing:same&&state.playing,currentTime:player.currentTime||0,duration:Number.isFinite(player.duration)?player.duration:0,volume:player.volume})};
  const toggle=item=>state.id===item.id&&state.playing?(ensureAudio().pause(),Promise.resolve()):start(item,'full');
  const playAt=(item,ratio)=>{cancelPending();const player=ensureAudio(),source=sourceOf(item),apply=()=>{const duration=Number.isFinite(player.duration)?player.duration:0;if(duration>0)player.currentTime=Math.max(0,Math.min(1,ratio))*duration;previewStartedAt=player.currentTime||0;publish({id:item.id,mode:'full',playing:false,currentTime:player.currentTime||0,duration,volume:player.volume});Promise.resolve(player.play()).catch(()=>publish({playing:false}))};if(state.id!==item.id||player.src!==source){player.pause();player.src=source;player.addEventListener('loadedmetadata',apply,{once:true});player.load?.()}else apply()};
  ...
  const seekBy=(item,seconds)=>{const player=ensureAudio();if(state.id!==item.id||state.mode!=='full')prepareFull(item);const duration=Number.isFinite(player.duration)?player.duration:0,next=Math.max(0,Math.min(duration||Infinity,(player.currentTime||0)+Number(seconds||0)));player.currentTime=next;publish({currentTime:next,duration})};
  const setVolume=value=>{const player=ensureAudio();player.volume=Math.max(0,Math.min(1,value));publish({volume:player.volume})};
  return {hover,leave,openFull,prepareFull,toggle,playAt,seek,seekBy,setVolume,stop,getState:()=>state,subscribe(listener){listeners.add(listener);listener(state);return()=>listeners.delete(listener)},getAudio:()=>audio};
}
