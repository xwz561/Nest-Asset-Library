...
app.whenReady().then(async () => {
  ...
  const html = `<style>html,body{margin:0;width:100%;height:100%;overflow:hidden;background:transparent}img{display:block;width:100%;height:100%}</style><img src="data:image/svg+xml;base64,${Buffer.from(svg).toString('base64')}">`;
  await win.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(html)}`);
  const image = await win.webContents.capturePage({ x: 0, y: 0, width: 1024, height: 1024 });
  win.destroy();
  if (image.isEmpty()) throw new Error(`无法渲染图标：${source}`);
  ...
  fs.copyFileSync(path.join(iconset, 'icon_512x512.png'), path.join(root, 'build', 'icon.png'));
